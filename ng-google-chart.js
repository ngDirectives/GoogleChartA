...
(function() {
    ...
    function GoogleChartController($scope, $element, $attrs, $injector, $timeout, $window, $rootScope, GoogleChartService) {
        ...
        function setupAndDraw() {
            //#region "Change font size code for vAxis and hAxis" -- Vikas 05/05/2016
            if (self.chart.data.rows.length >= 8 || self.chart.data.cols.length >= 8) {
                var vAxisFontStyle = {
                    textStyle: {
                ...
                angular.element.extend(self.chart.options.vAxis, vAxisFontStyle);
                angular.element.extend(self.chart.options.hAxis, hAxisFontStyle);
            }
            //#endregion
